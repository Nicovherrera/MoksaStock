...
public class ConfeccionServicio {
	...
	public void registroConfeccion (Date fecha, String nombreProveedroCcion, Double precio) throws ErrorServicio {
		
		validarC (fecha, nombreProveedroCcion, precio);
		
		Confeccion conf = new Confeccion();
		
		conf.setFechaAltaCcion(fecha);
		conf.setFechaAltaAntCcion(fechaUltimaCompraCcion(nombreProveedroCcion, fecha));
	...
	public Date fechaUltimaCompraCcion (String nombreProveedroCcion, Date fecha) {
		
		List<Confeccion> confL = confeccionRepo.buscarCcionxNombre(nombreProveedroCcion);
		
	    for (Confeccion conf : confL) {
            
	    	if (conf.getFechaAltaCcion().before(fecha)) {
	...
	public Double inflacionCcion(Confeccion conf){
		...
		String fechaActual=d2.format(conf.getFechaAltaCcion());
		...
		Double variacion= ((conf.getPrecioCompAntCcion()/conf.getPrecio())-1)*-100;
	...
	public void validarC (Date fecha, String nombreProveedroCcion, Double precio)throws ErrorServicio {
		
		
		if (fecha == null) {
			throw new ErrorServicio ("la fecha no puede estar vacia o nula");
			}
	...
	public void eliminarConfeccion (String id) {
		...
		Confeccion ccionEliminado=confeccionRepo.buscarConfeccionxId(id);
		
		List<Confeccion>ccionEstado=confeccionRepo.buscarCcionxNombre(ccionEliminado.getNombreProveedroCcion());
		
		if (ccionEstado.size()>1) {
		Confeccion ccionCambiado = ccionEstado.get(1);
		ccionCambiado.setEstadoCcion(true);
		}
