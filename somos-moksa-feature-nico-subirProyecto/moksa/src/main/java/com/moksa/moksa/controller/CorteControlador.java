...
public class CorteControlador {
	...
	public String registroCorte (ModelMap mapaC, String fecha, String nombreProveedroCte, Double precio ) throws ErrorServicio, ParseException {
		
	try {	
		SimpleDateFormat fechaFormateada = new SimpleDateFormat("yyyy-MM-dd");
		Date fechaCompra = fechaFormateada.parse(fecha);
		
		corteServicio.registroCorte(fechaCompra, nombreProveedroCte, precio);
		
		return "redirect:/corte#lista";
		
	}catch(ErrorServicio e) {
		
		mapaC.put("errorCte", "ha ocurrido una macana, debe haber omitido algun campo obligatorio");
