...
public class ConfeccionControlador {
	...
	public String registroConfeccion (ModelMap mapaCcion, String fecha, String nombreProveedroCcion, Double precio) throws ErrorServicio, ParseException {
		
		try {
			SimpleDateFormat fechaFormateada = new SimpleDateFormat("yyyy-MM-dd");
			Date fechaCompra = fechaFormateada.parse(fecha);
			
			servicioConf.registroConfeccion(fechaCompra, nombreProveedroCcion, precio);
			mapaCcion.put("exitoA", "La carga se realizo con exito ");
			return "redirect:/confeccion#lista";
		}catch (ErrorServicio e){
			mapaCcion.put("errorCcion", " Ocurrio un error, seguramente omitido algun dato obligatorio " );
			return "redirect:/confeccion#carga";
		}
