...
public class AvioServicio {
	...
	public void registroAvio(Date fecha, String nombreAvio, String tipoAvio, Float precio, String proveedor, Integer cantidadCom)
		...
		avio.setFechaFactAnt(fechaUltimaCompraA(nombreAvio, fecha));
	...
	public Date fechaUltimaCompraA(String nombreAvio, Date fecha) {

		List<Avio> avioL = avioRepo.buscarAvioxNombre(nombreAvio);

		for (Avio avio : avioL) {

			if (avio.getFechaAltaA().before(fecha)) {
	...
	public Float inflacionAvio(Avio aviof) {
		...
		String fechaActual = d2.format(aviof.getFechaAltaA());
	...
	public void eliminarAvio(String id) {
		...
		Avio avioEliminado=avioRepo.buscarxId(id);
		
		List<Avio>aviosEstado=avioRepo.buscarAvioxNombre(avioEliminado.getNombreAvio());
		
		if(aviosEstado.size()>1) {
		Avio avioCambiado = aviosEstado.get(1);
		avioCambiado.setEstadoA(true);
		}
